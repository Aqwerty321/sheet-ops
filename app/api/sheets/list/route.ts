...
/**
 * List spreadsheets shared with the service account
 * GET /api/sheets/list
 */
export async function GET() {
    try {
        ...
        // List files in service account's drive (shared files)
        const response = await fetch(
            `${DRIVE_API_BASE}/files?q=mimeType='application/vnd.google-apps.spreadsheet'&orderBy=modifiedTime desc&pageSize=20`,
            {
                headers: {
        ...
        return NextResponse.json(
            { error: "Failed to list sheets", details: String(error) },
            { status: 500 }
        );
    }
}
